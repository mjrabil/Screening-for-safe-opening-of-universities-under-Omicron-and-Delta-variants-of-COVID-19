...
#include "omicron_varying_boosted_constants.h"
#include "omicron_varying_boosted.h"
#include <iostream>
#include <iomanip>
#include <array>
...
int main()
{
   ...
   // myfile_infections.open("TEST0_Case_omega_not_0.txt");
    ...
    double initial_asymptomatic_N_S = 0; // 45; // 45;
    double initial_asymptomatic_N_F = 0; // 3; // 3;
    double initial_asymptomatic_W_S = 0;// 45; // 45;
    double initial_asymptomatic_W_F = 0; // 3; // 3;
    double initial_asymptomatic_B_S = 45;// 45;// 45; // 45;
    double initial_asymptomatic_B_F = 3;// 3;// 3; // 3;

    //Screening arrays:  { 0 }; // 
    double TAU_N_array[] = { 1.0 / (3 * 1.0) };// { 0, 1.0 / (3 * 14.0), 1.0 / (3 * 7.0), 1.0 / (3 * 3.0), 1.0 / (3 * 2.0), 1.0 / (3 * 1.0) };
    double TAU_W_array[] = { 1.0 / (3 * 1.0) };//{ 0, 1.0 / (3 * 14.0), 1.0 / (3 * 7.0), 1.0 / (3 * 3.0), 1.0 / (3 * 2.0), 1.0 / (3 * 1.0) };
    double TAU_B_array[] = { 1.0 / (3 * 1.0) };//{ 0, 1.0 / (3 * 14.0), 1.0 / (3 * 7.0), 1.0 / (3 * 3.0), 1.0 / (3 * 2.0), 1.0 / (3 * 1.0) };

    double LAMBDA_array[] = { 0 };// { 0, 1.0 / (3 * 6.0 * 7) };// { 0, 1.0 / (3 * 5.0 * 7), 1.0 / (3 * 6.0 * 7), 1.0 / (3 * 7.0 * 7) };

    
    double initial_U_B_S_array[] = { 45, 60, 90, 500, 1000, 2000,2250, 3000, 4000, 4500, 5000, 6000, 7000, 8000, 8500, 9000, 10000, 11000, 11250, 12000, 13000, 14000, 14500, 15000, 15750, 16000, 16875, 17000, 18000, 18500 }; //, 19000, 19125, 20000, 20250, 21000, 21375, 21500, 22000, 22250, 22400, 22500};
    double initial_U_B_F_array[] = { 3, 4, 6, 33, 67, 133, 150, 200, 267, 300, 333, 400, 467, 533, 567, 600, 667, 733, 750, 800, 867, 933, 967, 1000, 1050, 1067, 1125, 1133, 1200 , 1233}; // , 1275, 1267, 1333, 1350, 1400, 1425, 1433, 1467, 1483, 1493, 1500}; 

    

    double initial_U_N_S_array[] = { 4000}; //{4000, 90, 45, 0};//{4000, 7000, 10000 };
    double initial_U_N_F_array[] = { 267}; //{267, 6, 3, 0};// { 467 }; // {267, 467, 667 };

    //double initial_U_W_S_array[] = { 0 }; //{4000, 90, 45, 0};//{4000, 7000, 10000 };
    //double initial_U_W_F_array[] = { 0 }; //{267, 6, 3, 0};// { 467 }; // {267, 467, 667 };


    //For sensitivity analysis later:
    double ETA_array[] = { 1 }; // { 0.75, 0.9, 1}; // { 0.5, 0.75, 0.9, 0.95, 1 };    //compliance to screening
    double GAMMA_array[] = { 0.5 }; // { 0.5, 0.75, 0.95 };    //mask
    double OMEGA_array[] = { 0.95 }; // { 0.5, 0.75, 0.95 };    //proportion of omicron
    ...
    int initial_U_B_S_array_size = sizeof(initial_U_B_S_array) / sizeof(initial_U_B_S_array[0]);
    int initial_U_B_F_array_size = sizeof(initial_U_B_F_array) / sizeof(initial_U_B_F_array[0]);
    ...
    myfile_infections_table << "% Omicron \t" << "U_N,F(0) \t" << "U_N,S(0) \t" << "U_W,F(0) \t" << "U_W,S(0) \t" << "U_B,F(0) \t" << "U_B,S(0) \t" << " Scr compl (eta) \t" << "Mask reduction rate (gamma) \t" << "weeks until eligible \t" << "Tau_B \t" << "Tau_W \t" << "Tau_N \t";
    ...
    for (int o = 0; o < OMEGA_array_size; o++)
    {
       ...
       // {
            for (int k = 0; k < initial_U_B_S_array_size; k++)
            {
            for (int kn = 0; kn < initial_U_N_S_array_size; kn++)
              {
                for (int e = 0; e < ETA_array_size; e++)
                {
                    for (int a = 0; a < GAMMA_array_size; a++)
                    {
                        for (int p = 0; p < LAMBDA_array_size; p++)
                        {
                            ...
                            {
                                for (int w = 0; (w < TAU_W_array_size) && (w <= q); w++)
                                {
                                    for (int b = 0; (b < TAU_B_array_size) && (b <= w); b++)
                                    {
                                        TAU_N = TAU_N_array[q];
                                        TAU_W = TAU_W_array[w];//
                                        TAU_B = TAU_B_array[b];//
                                        U_B_S_0 = initial_U_B_S_array[k];
                                        U_B_F_0 = initial_U_B_F_array[k];
                                        U_N_S_0 = initial_U_N_S_array[kn];
                                        U_N_F_0 = initial_U_N_F_array[kn];

                                       // U_W_S_0 = initial_U_W_S_array[n];
                                        //U_W_F_0 = initial_U_W_F_array[n];

                                        U_W_S_0 = 22500 - U_B_S_0 - U_N_S_0;
                                        U_W_F_0 = 1500 - U_B_F_0 - U_N_F_0;
                                        ...
                                        for (int i = 0; i < loop_index; i++)
                                        {
                                            ...
                                            //but also if we change something in "it", the region in the region_list will change
                                               ...
                                               /* it->infections_N_S = it->uninfected_N_S * (BETA_NS_NS_t * it->ratio_N_S + BETA_NF_NS_t * it->ratio_N_F + BETA_WS_NS_t * it->ratio_W_S + BETA_WF_NS_t * it->ratio_W_F + BETA_BS_NS_t * it->ratio_B_S + BETA_BF_NS_t * it->ratio_B_F);
                                                it->infections_N_F = it->uninfected_N_F * (BETA_NS_NF_t * it->ratio_N_S + BETA_NF_NF_t * it->ratio_N_F + BETA_WS_NF_t * it->ratio_W_S + BETA_WF_NF_t * it->ratio_W_F + BETA_BS_NF_t * it->ratio_B_S + BETA_BF_NF_t * it->ratio_B_F);
                                                it->infections_W_S = it->uninfected_W_S * (BETA_NS_WS_t * it->ratio_N_S + BETA_NF_WS_t * it->ratio_N_F + BETA_WS_WS_t * it->ratio_W_S + BETA_WF_WS_t * it->ratio_W_F + BETA_BS_WS_t * it->ratio_B_S + BETA_BF_WS_t * it->ratio_B_F);
                                                it->infections_W_F = it->uninfected_W_F * (BETA_NS_WF_t * it->ratio_N_S + BETA_NF_WF_t * it->ratio_N_F + BETA_WS_WF_t * it->ratio_W_S + BETA_WF_WF_t * it->ratio_W_F + BETA_BS_WF_t * it->ratio_B_S + BETA_BF_WF_t * it->ratio_B_F);
                                                it->infections_B_S = it->uninfected_B_S * (BETA_NS_BS_t * it->ratio_N_S + BETA_NF_BS_t * it->ratio_N_F + BETA_WS_BS_t * it->ratio_W_S + BETA_WF_BS_t * it->ratio_W_F + BETA_BS_BS_t * it->ratio_B_S + BETA_BF_BS_t * it->ratio_B_F);
                                                it->infections_B_F = it->uninfected_B_F * (BETA_NS_BF_t * it->ratio_N_S + BETA_NF_BF_t * it->ratio_N_F + BETA_WS_BF_t * it->ratio_W_S + BETA_WF_BF_t * it->ratio_W_F + BETA_BS_BF_t * it->ratio_B_S + BETA_BF_BF_t * it->ratio_B_F);*/
